...
def show_answer(answer, selected_word, missing_letter_index):
    ...
    elif answer != selected_word [missing_letter_index ]:
        print("The word was: "+selected_word)
...
def ask_file_name():
    ...
    default_file="short_words.txt"
    
    #determines if the user just hit enter in order to select the default file
    select_file = input("Words file? [leave empty to use short_words.txt] : ")
    if select_file == "":
        pass
        return default_file.strip('')
    else :
        return select_file
